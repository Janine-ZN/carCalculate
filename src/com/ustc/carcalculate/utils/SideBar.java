...
public class SideBar extends View {
	...
	public static String[] chars = { "#", "A", "B", "C", "D", "E", "F", "G",
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < chars.length; i++) {
			paint.setColor(Color.parseColor("#888888"));// 索引条字母的颜色#d9d9d9
			paint.setTypeface(Typeface.DEFAULT_BOLD);// 设置粗体
			paint.setAntiAlias(true); // 抗锯齿
			paint.setTextSize(24);
			if (chooseIndex == i) {
				paint.setColor(Color.parseColor("#9fcefc"));
			}
			// 绘制文本
	...
	public boolean dispatchTouchEvent(MotionEvent event) {
		...
		default:
			setBackgroundResource(R.drawable.side_bar_bg); // 设置背景
			// 根据y坐标获取点到的字母
			if (oldChoode != c) {
